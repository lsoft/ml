...
namespace MyNN.MLP.DropConnect.Backpropagation.EpocheTrainer.DropConnect.OpenCL.CPU
{
    /// <summary>
    /// Kernel source provider for dropconnect backpropagation epoche trainer with bit mask that enables CPU-OpenCL
    /// </summary>
            ...
            result =
                result.Replace(
                    ...
                    (Math.Abs(_config.RegularizationFactor) < float.Epsilon
                        ? "0"
                        : @"
здесь ошибка в регуляризации!!! смотри версию для CPU (читаем один флоат из currentLayerWeights[currentWeightIndex4] , а  не 4; надо использовать vload4)
