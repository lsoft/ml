...
{
    public class OpenCLTransposeBackpropagationAlgorithm : IEpocheTrainer
    {
        ...
        private IOpenCLTranposer[] _tranposers;
        ...
        public OpenCLTransposeBackpropagationAlgorithm(
            ...
            )
        {
            ...
            if (config.BatchSize == 1)
            {
                ...
                Console.WriteLine("This backpropagation algorithm optimized to work in batch mode (typical with batch size = [25;100]). Online backpropagation is not an optimal choise. Try to use OpenCLTranspose2BackpropagationAlgorithm.");
            }

            if (config.BatchSize > 1 && config.BatchSize < 25)
            {
                Console.WriteLine("========================================= WARNING =========================================");
                Console.WriteLine("Too low minibatch size ({0}) to achieve optimal performance. Try to increase batch size to 25 minimum.", config.BatchSize);
            }
        ...
        private void GenerateMems()
        {
            ...
            _tranposers = new IOpenCLTranposer[_mlp.Layers.Length];
        }
        ...
        public void PreTrainInit(DataSet data)
        {
            ...
            for (var i = 1; i < _mlp.Layers.Length; i++)
            {
                ...
                _tranposers[i] = new TranposerNvidia(
                    _clProvider,
            ...
            float learningRate)
        {
            ...
            for (var layerIndex = 1; layerIndex < _mlp.Layers.Length; ++layerIndex)
            {
                _tranposers[layerIndex].Tranpose();
            }
                    ...
                    for (int hiddenLayerIndex = _mlp.Layers.Length - 2; hiddenLayerIndex > 0; hiddenLayerIndex--)
                    {
                        ...
                        if (inBatchIndex == 0)
                        {
                            _hiddenKernelOverwrite[hiddenLayerIndex]
                                ...
                                .SetKernelArgMem(5, _tranposers[hiddenLayerIndex + 1].Destination)
