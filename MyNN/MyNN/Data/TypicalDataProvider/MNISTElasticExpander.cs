    ...
    public class MNISTElasticExpander
    {
            ...
            int stepRefreshGaussMap)
        {
            ...
            for (var dd = 0; dd < deformationEpocheCount; dd++)
            {
                ...
                for (var cc = 0; cc < originalImagesCount; cc++)
                {
                    var o = toDeformDataSet[cc];
            ...
            )
        {
            ...
            for (var x = 0; x < scaledImageWidth; x += step)
            {
                ...
                {
                    var diffx = rnd.NextDouble() * 2.0 - 1.0;
                    rdfX[x, y] = diffx;

                    var diffy = rnd.NextDouble() * 2.0 - 1.0;
                    rdfY[x, y] = diffy;
                }
            }
            ...
            using (var src = IplImage.FromBitmap(origImage))
            {
                ...
                using (var gray = new IplImage(src.Size, BitDepth.U8, 1))
                {
                    ...
                    using (var rotated = new IplImage(src.Size, BitDepth.U8, 1))
                    {
                        ...
                        {
                            var centerShiftX = rnd.NextDouble()*20 - 10;
                            var centerShiftY = rnd.NextDouble()*20 - 10;
                            var angle = rnd.NextDouble()*(2*maxRotationAngle) - maxRotationAngle;
