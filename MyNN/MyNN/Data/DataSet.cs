...
namespace MyNN.Data
{
    ...
    public class DataSet : IEnumerable<DataItem>, IVisualizer
    {
        ...
        public DataSet(IVisualizer visualizer = null)
        {
            Visualizer = visualizer;
            Data = new List<DataItem>();
        }

        public DataSet(List<DataItem> data, IVisualizer visualizer = null)
        {
            #region validate
        ...
        public DataSet(DataSet dataSet, List<float[]> inputPart, IVisualizer visualizer)
        {
            if (dataSet == null)
            {
                throw new ArgumentNullException("dataSet");
            ...
            ref int rndSeed)
        {
            ...
            for (var epocheNumber = 0; epocheNumber < epocheCount; epocheNumber++)
            {
                ...
                foreach (var d in this.Data)
                {
                    ...
                    {
                        var linearcoef = (1.0f - (((float)rnd.NextDouble()) * scale / 2.0f - scale));
        ...
        {
            var random = new Random((int) (DateTime.Now - new DateTime(2000, 1, 1)).TotalSeconds);
            ...
            for (int i = 0; i < cloned.Count - 1; i++)
            {
                ...
                {
                    var newIndex = random.Next(cloned.Count);
            ...
            return
                new DataSet(cloned, this.Visualizer);
        }
        ...
        public DataSet CreateBinarizedDataSet()
        {
            ...
            foreach (var di in this.Data)
            {
                ...
                var bi = di.Input.ToList().ConvertAll(j => (random.NextDouble() < j) ? 1f : 0f);
